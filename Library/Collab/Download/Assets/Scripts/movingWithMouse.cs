...
public class movingWithMouse : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            rotating = false;
            rotX += Input.GetAxis("Mouse Y") * rotSpeed * sensitivity * Mathf.Deg2Rad * Time.deltaTime;
            rotY -= Input.GetAxis("Mouse X") * rotSpeed * sensitivity * Mathf.Deg2Rad * Time.deltaTime;
        }
        ...
        if (rotating || returning)
        {
            ...
            else
            {
                ...
                if (rotating) //ESTO ES PORQUE YA NO SÉ QUÉ PUTO HACER Y ESTOY LLORANDO
                {
                    transform.DetachChildren(); //ESTO ES PORQUE YA NO SÉ QUÉ PUTO HACER Y ESTOY LLORANDO
                }
                else if (returning) //EL ELSE ES PORQUE YA NO SÉ QUÉ PUTO HACER Y ESTOY LLORANDO
                {
                    //hub.transform.Rotate(nextAngle - transform.rotation.eulerAngles); //ESTO NO VA
                    hub.transform.SetParent(transform); //ESTO ES PORQUE YA NO SÉ QUÉ PUTO HACER Y ESTOY LLORANDO
                    //hub.transform.rotation = newRot; //ESTO VA AÚN PEOR SI CABE
                }
            }
            initialState = transform.rotation;
        }
